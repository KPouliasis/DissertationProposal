...
and make a case that such extensions are of interest from the trinitarian (logic/ type theory/ category theory)
point of view.
...
Heyting algebras we could have a stack of Heyting algebras related with order preserving
...
but can be found in the excellent \cite{}. We also require a that tells now 
...
\begin{mdframed}[nobreak=true, frametitle={Reduction rules on justifications }]
\mbox{\footnotesize
    \begin{mathpar}
        \inferrule*[right=$I_{\Box B}E^{\vec{x},\vec{s}}_{\Box A_1\ldots \Box A_i}$]
        {{(\forall  A_i \in \Gamma'. \ \Turn {\Gamma}{\Box  A_i})}\\{\Turn {\Gamma'} { B}}\\{\Turn {F_i\Gamma'} {F_i  B} }} {\Turn {\Gamma}\Box  B}
...
\subsection {Factivity and adjunctions}
