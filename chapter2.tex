...
\begin{mathpar}
\inferrule*[right=InjL] {\Turnsi {\Gamma} {M:A}} {\Turnsi {\Gamma} {  inj_l[B](M):A+B}}
\and
\inferrule*[right=InjR] {\Turnsi {\Gamma} {M:B }} {\Turnsi {\Gamma} {inj_r[A](M) : A+B}}
\end{mathpar}
...
Local soundness captures the idea that  ``elimintation rules are not too strong" or ``local soundness", which is the informal notion that the 
...
\begin{mdframed}
	\begin{mathpar}
		...
		{ {\Gamma,x
				:A\vdash N:\psi}\\{\Gamma,y:B \vdash O:\psi}\\{\Gamma\vdash P:A} }
		{\Gamma\vdash  ({\text{case } inj_l(P) \text{ of } inj_l(x) \longmapsto N  |\  inj_r (y)\longmapsto O) =[P/x]N: \psi}}
		\and
		\inferrule*[right=$\beta\vee_2$]
		{ {\Gamma,x
				:A\vdash N:\psi}\\{\Gamma,y:B \vdash O:\psi}\\{\Gamma\vdash Q:B} }
		{\Gamma\vdash  ({\text{case } inJ_r(Q) \text{ of } inj_l(x)  \longmapsto N  |\  inj_r(y) \longmapsto O) =[Q/y]O: \psi}}
	\end{mathpar}
\end{mdframed}
...
\begin{mdframed}
	\begin{mathpar}
		...
		\and
		\inferrule*[right=$\eta\rightarrow$]{\Gamma\vdash M:A\rightarrow B}{\Gamma\vdash M= \lambda x. M x: A\rightarrow B} 
		\and
