...
See also discussion in \cite{Bellin2001,bierman2000intuitionistic}). We will be referring to this rule  as ``$\Box$ Intro--After--Elim'' or, simply $\Box_{IE}$, from now on.
...
%	\end{threorem}
	...
	is given elsewhere (e.g. \cite{bo:prawitz65a}) and in Gentzen's original \cite{gentzen1935untersuchungen}. Gentzen's program can be described with the following two slogans:\begin{itemize} \item[a.] Elim is left-inverse to Intro \item[b.] Intro is right-inverse to Elim\end{itemize}   
		...
		\begin{mdframed}[nobreak=true,frametitle={\footnotesize Sequent Calculus ({\sf Prop})}]
			...
			\begin{mdframed}[nobreak=true,frametitle={\footnotesize Judgments on Truth  $\Gamma, A,B \in {\sf Prop_0}$  and $M := x_i\  |\  <> \ |\ \lambda x:A.\ M\  |\  (M M) $}]
				...
				\mbox{\small
					...
					important results on programming paradigms related to justification logic have been obtained in \cite{ArtBon07LFCS,bavera2015justification}. 
					...
					direction have been developed in \cite{Pouliasis201471}. 
