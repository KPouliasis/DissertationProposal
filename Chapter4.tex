	...
	\mbox{\small
		...
		\begin{mdframed}[nobreak=true,frametitle={\footnotesize Sequent Calculus ({\sf Prop})}]
			...
			\end{mdframed}
				...
				$\Gamma\vdash_{Jcal}\phi$ iff for any \vocab{Jcalc Algebra} $JC$ ($HA,F,J$)we have 
				$\Gamma^+\leq\phi^{*}$ where $*$ is  defined as 
				the extension of any map of atomic $\prop$s ($p_i$) to elements of $HA$ and $(+)$ 
				is defined inductively on the length of $\Gamma$.
				\begin{alignat*}{2}
					(\top)* &&\quad= & \quad\top\\
			...
			\begin{proof}
				...
				mapping $*$ as described below then by case analysis: 
				\begin{enumerate}
					\item $\Gamma,\phi\in {\sf Prop_0}, \llbracket{\sf Prop_0}\rrbracket $ 
