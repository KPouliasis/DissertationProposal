\chapter{Proposal for Thesis contribution}\label{proposal}
...
In his seminal ``Explicit Provability and Constructive Semantics'' \citelatex{Artemov2001} Artemov developed a constructive, proof-theoretic semantics for 
\acs{BHK} proofs \citelatex{troelstra1988constructivism} 
...
(e.g. a classical axiomatization of Peano arithmetic, a classical or intuitionistic set theory etc). % In Artemov's work $I$ is assumed to be
%based on intuitionistic logic and $J$  on classical logic. %We, initially,  mute such assumptions to focus exclusively on the mechanics of necessity in this framework.
...
Curry--Howard setting \citelatex{ArtBon07LFCS}. Our paper
focuses on  $K$ modality and  should be viewed as the  counterpart of \citelatex{Bellin2001} with justifications as we explain in \ref{relat}.
\subsection{Deductive Systems, Validity and Necessity}
Following a framework championed by Lambek \citelatex{lambek1968deductive,lambek1969deductive}, let us  assume two deductive systems $I$ 
...
\begin{definition}Given a deductive system $I$, an   \textit{interpretation for $I$}, noted by $\llbracket\bullet \rrbracket_J$, is a pair
	...
	\begin{enumerate}
		...
		As in \citelatex{lambek1968deductive} they can be treated as 
		the $\top$ element).
	\end{enumerate}
\end{definition}
...
\end{definition}
Note, that we require existence but not uniqueness. Nevertheless, if we treat deductive systems  in a proof irrelevant manner as preorders 
...
endofunctorial interpretations of $K$ modality appearing in \citelatex{Bellin2001},\citelatex{kavvos2016system}. 
...
See also discussion in \citelatex{Bellin2001,bierman2000intuitionistic}). We will be referring to this rule  as ``$\Box$ Intro--After--Elim'' or, simply $\Box_{IE}$, from now on.
%Moreover, it has a very clear computational reading in our system which we will be explaining. In a nutshell, theorems of $K$ correspond to programs that consume (eliminate) congruences on subterms to construct construct congruences on larger terms. In that sense, the elim-intro mix is quite natural.
...
\mbox{
	...
	The rules below should be obvious except for small caveat. On the one hand, the type universe of $U_I$ and the proof trees of $I$ are 
	...
	\begin{mdframed}[nobreak=true,frametitle={\footnotesize Judgments on Type Universe(s)}]
		\mbox{\small
			\begin{mathpar}
				...
				\and
				\inferrule*[right= Arr$_0$] {{ A \in {\sf Prop_0 }}\\ { B \in {\sf Prop_0}}} { A\supset  B\in {\sf Prop_{0}}}
				\and
				\inferrule*[right= Arr$_1$] {{ A \in {\sf Prop_1 }}\\ { B \in {\sf Prop_1}}} { A\supset  B\in {\sf Prop_{1}}}
				\and
...
\begin{mdframed}[nobreak=true,frametitle={\footnotesize Hilbert Style Formulation}]
	...
	The steps of the proof are given in the Appendix, but this is essentially  the ``lifting lemma'' in justification logic \citelatex{Artemov2001}. 
	The proof fully depends on
	the provability requirements imposed in the $\llbracket{\sf Prop_0}\rrbracket$ fragment.
	\begin{restatable}[$\Box$Lifting Lemma]{lemma}{boxlift}
	...
	\begin{restatable}[Admissibility of Necessitation]{lemma}{firstadm}
	...
	is given elsewhere (e.g. \citelatex{bo:prawitz65a}) and in Gentzen's original \citelatex{gentzen1935untersuchungen}. Gentzen's program can be described with the following two slogans:\begin{itemize} \item[a.] Elim is left-inverse to Intro \item[b.] Intro is right-inverse to Elim\end{itemize}   
	...
	We show here the ``Elim-After-Singleton-Intro" sub-case. The exact same principle applies in the ``Elim-after-Intro''  of   multiple $\Box$s shown in the Appendix (\ref{redmult}).
	...
	\mbox{\small
		...
		\begin{mdframed}[nobreak=true,frametitle={\footnotesize Sequent Calculus ({\sf Prop})}]
			...
			%\end{proposition}
			\begin{restatable}[Consistency of {\sf Jcalc$^{-}$}]{theorem}{firstcon}
				$\centernot{\vdash}\bot$
			\end{restatable}
			...
			\begin{mdframed}[nobreak=true,frametitle={\footnotesize Judgments on Truth  $\Gamma, A,B \in {\sf Prop_0}$  and $M := x_i\  |\  <> \ |\ \lambda x:A.\ M\  |\  (M M) $}]
				...
				\mbox{\small
					...
					\begin{mdframed}[nobreak=true,frametitle={\footnotesize Small step, call-by-value reduction $\rightarrow$}]
						...
						\end{mathpar}
					\end{mdframed}
					Using the reducibility candidates proof method \citelatex{citeulike:993095}) we show:
					...
					\begin{itemize}
						...
						from the embedding on intuitionistic logic to intuitionistic linear logic \citelatex{girard1987linear}.
					...
					\label{relat}
					Directly related work with our calculus, in the same fashion that justification logic and LP \citelatex{Artemov2001} are related to modal logic, is \citelatex{Bellin2001}.  The work in \citelatex{Bellin2001} provides a calculus for explicit assignments (substitutions) which is actually a sub-case of 
					{\sf Jcalc$^{-}$} with $\llbracket\bullet \rrbracket$ identity. This  sub-case  captures dynamic linking where the host language is the very same one; such need appears in languages with module mechanisms (i.e. implementation hiding and separate compilation within the very same language). In general, the judgmental approach to modality is heavily influenced by \citelatex{citeulike:5447115}. In a sense, our treatment of validity-as-explicit-provability also generalizes the approach there without having to commit to a ``factive" modality. Finally,  
					important results on programming paradigms related to justification logic have been obtained in \citelatex{ArtBon07LFCS,bavera2015justification}. 
					...
					direction have been developed in \citelatex{Pouliasis201471}. 
					%\nocite{Pfenning2009a, Pfenning2009b}
					
					\bibliographystylelatex{plainnat}
					\bibliographylatex{jcalcminustex}
