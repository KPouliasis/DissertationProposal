...
\begin{frame}{Example: rules $\supset$}
  \begin{mathpar}
  \inferrule[]{{\begin{array}[b]{c} \overline{x:A}\\ {\mathcal{D}} \\ A   \end{array}} }{A\supset B }
  \and
...
\begin{frame} {Natural deduction for ${\sf \llbracket Prop_0\rrbracket}$}
    \begin{mathpar}
      \inferrule*[right=$\Delta$-Refl] {s:\llbracket  A\rrbracket \in \Delta}{\Turnsi {\Delta} {\llbracket  A\rrbracket}}
...
\[\begin{array}{c} \inferrule*{x^{\prime}: \Box A \in \Gamma}{ {\Gamma}\vdash {\Box A}} \\ \\ \inferrule*{{(\forall A_i \in \Gamma^{\prime}. \ {\Gamma}\vdash{\Box A_i})}\\{{\Gamma^{\prime}}\vdash { B}}\\ { {\llbracket \Gamma^{\prime} \rrbracket}\vdash {\llbracket B\rrbracket} }} { {\Gamma}\vdash\Box B} \\ \\ \inferrule* {{ {\Gamma, x^{\prime}: \Box A} \vdash { \Box B}}} { {\Gamma}\vdash { \Box A\supset \Box B}} \\ \\ \inferrule* {{ {\Gamma} \vdash { \Box A\supset \Box B}}\\{ {\Gamma} \vdash { \Box A}}} { {\Gamma}\vdash { \Box B}} \end{array} \]
  \begin{mathpar}
    \inferrule*[right=$\wedge_0$I] {{\Turnsi {\Gamma} { \Box A}}\\{\Turnsi {\Gamma} { \Box B}}} {\Turnsi {\Gamma} {   \Box A\wedge \Box B}}
    \inferrule*[right=$\wedge_0$E1] {{\Turnsi {\Gamma} {\Box A\wedge \Box B}}} {\Turnsi {\Gamma} {  \Box A}}
    \inferrule*[right=$\wedge_0$E2] {\Turnsi {\Gamma} {\Box A\wedge \Box B}} {\Turnsi {\Gamma} {   \Box B}}
  \end{mathpar}
\end{frame}
...
\end{frame} 
 \begin{frame}
  ...
  \begin{frame}
   ...
   \[\begin{array}{l l} {\sf pop (push\ 2 \ empty)} \& \\ {\sf remove\_last*(add\_array*\ 2 * create\_array())} \end{array} \]
  ...
  \end{columns}
    ...
    \begin{frame}{From order theory to category theory?}
      \begin{itemize}
      
      \item it requires the symbol  (or,$\llbracket\bullet\rrbracket$) to be interpeted as a "good" (preserving connectives) functor
      \item the source of the semantical triplet  would be a  Cartesian Closed Category (instead of a heyting algebra)
      \item the rule would look like this 
      \begin{mathpar}
        \inferrule*[right=$I_{\Box B}E^{\vec{x},\vec{s}}_{\Box A_1\ldots \Box A_i}$] %{}
        {{\forall A_k \in \Gamma^\prime . \   \Gamma \vdash N_k:\Box A_k}\\
            {\Turnsi {\Gamma^\prime} {M:B}}\\{\Turnsi {\llbracket\Gamma^\prime\rrbracket} {{\sf \llbracket M\rrbracket}:\llbracket B \rrbracket} }} 
        {\Turnsi {\Gamma} {{\sf let^{*}}\ (\Gamma^\prime, \llbracket\Gamma^\prime\rrbracket, Ns) \ {\sf in}  \ ( M\& {\sf \llbracket M\rrbracket}):\Box B}}
    \end{mathpar}
    \end{itemize}
  \end{frame}
   \begin{frame}{From order theory to category theory?}
    \begin{itemize}
      \item A ccc models equalities between terms (as equalities between arrows)
      \item A functor betwenn cccs is a functional on arrows ($M=N \Rightarrow FM=FN$)
      \item Hence we need to talk about equality in the level of justifications
      \item the $SK$ calculus can be extended equationally to preserve the $\lambda$calculus computationally
      \item We need to add rules that axiomatize $\llbracket M\rrbracket$ on terms
    \end{itemize}
  \end{frame}
    ...
    \begin{mathpar}
        ...
        {\Gamma\vdash {\sf return} \ \  {\sf j} :R\phi} 
    \end{mathpar}
    ...
    \begin{frame}
      ...
      \begin{mathpar}
          \inferrule*[right=$R$]{\Box\Gamma\vdash M:\Box\phi}
          {\Box\Gamma\vdash \sf {let} \ \_\_ \ \&\ s \ {\sf be\ } M \ \  {\sf in}\  {\sf return} (s):R\llbracket\phi\rrbracket} 
      \end{mathpar}
