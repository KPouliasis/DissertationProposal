...
\begin{frame}{Metatheory for (almost) free}
  \begin{outline}
    \1 []  The good old  way
        \2[!] Forget about variables/ terms/ equalities and prove that $\Gamma\vdash A$ has an order theoretic model

    \1[] Intercalation and Cut elimination (core of CHI)
      \2[!]  Create a restricted version that:
      \3[!!] does not permit (Elim - After- Intro) 
      \3[!!] does not permit Reflection on hypothesis in non-atomic formulas
   ...
   \end{frame}
    ...
    \begin{frame}
      ...
      \begin{mathpar}
          ...
          {\Gamma\vdash \sf {let} \ \_\_ \ \&\ s \ {\sf be\ } M \ \  {\sf in}\  {\sf return} (s):R\llbracket\phi\rrbracket} 
      \end{mathpar}  
    \end{frame}  
  \end{document}
